...
function getArtistByIndex(arry,index) {
  return `the artist at index ${arry[index].id} is ${arry[index].name}`;
  /*Your Code Here*/
}  
...
function get20s(arry){
  ...
  for(let i = 0;i<arry.length;i++){
    let str = arry[i].years;
    const born = Number(str.slice(0,str.indexOf(" ")));
    str = str.slice(str.lastIndexOf(" ")+1);
    const died = Number(str);
    if(born>=1900&&died<=2000){
      death.push(arry[i].name);
    }
  }
  return death;
  /*Your Code Here*/
...
function randomize(arry){
    let copy = [...arry];    // so original data woudn't be affected
    let rand = [];
    while(copy.length!==0){
      const randOne = Math.floor(Math.random()*copy.length);
      rand.push(copy[randOne]);
      copy.splice(randOne,1);
    }
    return rand;
    /* Code here */
